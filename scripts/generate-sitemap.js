...
const BASE_URL = 'https://inova-sv.vercel.app';
...
async function generateSitemap() {
  ...
  if (SUPABASE_URL && SUPABASE_KEY) {
    try {
      ...
      const response = await fetch(`${SUPABASE_URL}/rest/v1/products?select=slug,updated_at&is_active=eq.true`, {
        headers: {
          'apikey': SUPABASE_KEY,
      ...
      if (response.ok) {
        ...
        products.forEach(p => {
          if (p.slug) routes.push({ path: `/product/${p.slug}`, lastmod: p.updated_at });
        });
...
${routes
  .map(
    ...
    <priority>${route.path === '/' ? '1.0' : route.path.startsWith('/product/') ? '0.7' : '0.8'}</priority>
  </url>`
  )
  .join('\n')}
</urlset>`;
